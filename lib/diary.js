...
Diary.prototype.setBreakfastTable = function () {
  ...
  if(typeof(currentDiaries[0]) === 'undefined'){
    currentDiaries[0] = {};
  }
  diary.breakfast.foods.forEach(function(food) {
    if(currentDiaries[0].breakfast == null) {
      currentDiaries[0].breakfast = []
    }
    currentDiaries[0].breakfast.push(food)
  })
...
Diary.prototype.setLunchTable = function () {
  ...
  diary.lunch.foods.forEach(function(food) {
    if(currentDiaries[0].lunch == null) {
      currentDiaries[0].lunch = []
    }
    currentDiaries[0].lunch.push(food)
  })
  diaryJSON = JSON.stringify(currentDiaries)
  localStorage.setItem('diary', diaryJSON)
  this.lunchTable();
...
Diary.prototype.setDinnerTable = function () {
  ...
  diary.dinner.foods.forEach(function(food) {
    if(currentDiaries[0].dinner == null) {
      currentDiaries[0].dinner = []
    }
    currentDiaries[0].dinner.push(food)
  })
  diaryJSON = JSON.stringify(currentDiaries)
  localStorage.setItem('diary', diaryJSON)
  this.dinnerTable();
...
Diary.prototype.setSnacksTable = function () {
  ...
  diary.snacks.foods.forEach(function(food) {
    if(currentDiaries[0].snacks == null) {
      currentDiaries[0].snacks = [];
    };
    currentDiaries[0].snacks.push(food);
  })
  diaryJSON = JSON.stringify(currentDiaries);
  localStorage.setItem('diary', diaryJSON);
  this.snacksTable();
...
Diary.prototype.setExerciseTable = function () {
  ...
  var currentDiaries = JSON.parse(diaryJSON);
  if(typeof(currentDiaries[0].exercises) === 'undefined'){
    currentDiaries[0].exercises = [];
  }
  diary.exercises.exercises.forEach(function(exercise) {
    if(currentDiaries[0].exercises == null) {
      currentDiaries[0].exercises = [];
    };
    currentDiaries[0].exercises.push(exercise);
  })
  diaryJSON = JSON.stringify(currentDiaries);
  localStorage.setItem('diary', diaryJSON);
  this.exerciseTable();
...
Diary.prototype.exerciseTable = function () {
  ...
  if(currentDiaries[0].exercises) {
    currentDiaries[0].exercises.forEach(function(exercise){
...
Diary.prototype.loadDiary = function () {
  var diaryJSON = localStorage.getItem('diary');
  if (diaryJSON === null) {
    diaryJSON = '[]';
  }
  currentDiaries = JSON.parse(diaryJSON);
  var currentDiary = currentDiaries[0] || {}
