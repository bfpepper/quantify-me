...
Diary.prototype.setDinnerTable = function () {
  var that  = this;
  var diary = new Diary();
  $('input[type=checkox]:checked').parent().each(function(index, that) {
    var name, calories;
    $(this).siblings().each(function(index, that) {
      if(index == 0) {
        name = $(this).text();
    ...
    var currentDiaries = JSON.parse(diaryJSON);
    currentDiaries.dinner = diary.breakfast.foods
    diaryJSON = JSON.stringify(currentDiaries)
    localStorage.setItem('diary', diaryJSON)
    this.dinnerTable();
  })
...
Diary.prototype.dinnerTable = function () {
  ...
  var dinnerJSON = localStorage.getItem('dairy');
  if (dinnerJSON === null) {
    dinnerJSON = '[]';
  }
  currentDiaries = JSON.parse(dinnerJSON);
