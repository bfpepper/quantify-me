...
Food.prototype.saveFood = function (name, calories) {
  var foodsJSON = localStorage.getItems('foods');
  if (foodsJSON === null) {
    foodsJSON = '[]';
  }
  var currentFoods = JSON.parse(foodsJSON);
  currentFoods.push({name: name, calories: calories})
...
Food.prototype.updateFood = function () {
  ...
  $('.update-food-name, .update-food-calorie-count').on('blur', function () {
    var name = $('input.update-food-name').val();
    var calories = $('input.update-food-calorie-count').val();
    var newFood = new Exercise;
    newExercise.updateLocal(oldName, name, calories)
  }
});

Food.prototype.updateLocal = function (oldNmae, name, calories) {
  var foodsJSON = localStorage.getItem('foods');
  if (foodsJSON === null) {
    foodsJSON = '[]'
  }
